...
WebView 하나마다 렌더러 프로세스가 하나 붙고, 그 고정 비용이 **약 100MB**다. 정작 메모
페이지가 쓰는 건 JS 힙 1.6MB·DOM 309개뿐이라(실측), **페이지를 가볍게 만드는 최적화는
의미가 없고 프로세스 수만이 레버**다. 메모 24장 기준 실측 2.38GB였다. 세 가지로 줄인다.

1. **감춘 창은 WebView를 만들지 않는다** (`Options::deferUntilShown`). `StickerWindow`·
   `GroupWindow`가 `show == false`면 이 옵션으로 `Create()`를 부르고, 실제 생성은 표시할 때
   `ShowWin` → `EnsureCreated()`가 한다(원래 크래시 복구용으로 있던 경로를 그대로 쓴다).
   비용이 *전체* 메모가 아니라 *바탕화면에 있는* 메모에만 붙는다. 2.38GB → 1.20GB.
   - 지연 생성은 init JSON을 굳게 만든다 — 감춘 메모는 몇 시간 뒤에 열리고 그 사이 테마·
     언어·서체가 바뀔 수 있다. 그래서 `SetInitProvider()`로 **생성 시점에** 다시 만든다.
     본문·제목은 원래부터 페이지가 `sticker.load`로 읽으므로 영향이 없다.
2. **렌더러 프로세스 상한** (`--renderer-process-limit`, 설치된 RAM의 GB/2를 3~8로 자름).
   우리 페이지는 모두 같은 출처(`app.sticker`)의 우리 코드라 프로세스를 나눠 써도 격리가
   약해지지 않는다(웹 메모가 여는 외부 사이트는 Chromium 사이트 격리가 그대로 지킨다).
   1.20GB → 0.88GB(상한 8) / 0.75GB(상한 4).
   - **대가**: 한 프로세스를 함께 쓰는 메모는 그중 하나가 오래 걸리는 일(큰 3D 모델 파싱
     등)을 하면 같이 멈춘다 — 실측으로 5장이 한 프로세스에 묶이면 그 5장이 함께 멎었다.
     그래서 흔한 사용(화면에 메모 몇 장)에서는 상한에 닿지 않게 넉넉히 잡고, RAM이 적은
     PC에서만 더 죈다.
3. **감춘 웹뷰는 메모리를 반납시킨다** (`ICoreWebView2_19::put_MemoryUsageTargetLevel`을
   `SetVisible`에서 LOW/NORMAL로). `IsVisible=FALSE`만으로는 **거의 줄지 않는다**(실측
   -6MB). 힌트를 주면 메모 10장을 감출 때 1020MB → 579MB(**-441MB**). 스크립트는 계속
   돌기 때문에 저장 타이머가 끊기지 않는다(그래서 `TrySuspend`는 쓰지 않는다).
...
### 캘린더 (리치 메모, `ui/editor/calendar.js`)

넣는 방법은 표와 같다(툴바 버튼 / 본문 빈 곳 우클릭). **표와 결정적으로 다른 점은 저장하는
것이 화면이 아니라 데이터라는 점이다.** 표는 칸 자체가 본문이지만, 캘린더의 격자는 날짜에서
계산해 그리는 그림이다. 그래서 남는 것은 `.mcal`의 속성 셋뿐이고 격자는 열 때 다시 그린다:

```html
<div class="mcal" contenteditable="false" data-view="month|week|day" data-date="2026-09-03"
     data-events='[{"id":"e1","d":"2026-09-03","t":"14:00","title":"회의"}]'></div>
```

- 안에 그려 넣는 것(머리글·격자·일정 편집기)은 전부 `data-chrome`이라 `getHtml`이 걷어낸다.
  **저장본을 다시 넣고 `renderAll()`만 부르면 격자와 일정이 그대로 되살아난다**(실측).
- `contenteditable="false"` — 캘린더 안은 타이핑하는 곳이 아니라 눌러서 다루는 곳이다.
- **보기**: 월간(주 수를 달에 맞춰 5~6줄) / 주간(7칸, 칸이 더 높다) / 일간(그 날짜의 목록).
  날짜 이름·요일·월 이름은 `toLocaleDateString`이 로케일에 맞춰 낸다 — 별도 번역표가 없다.
- **일정**: 빈 날짜 칸을 누르면 새 일정, 일정 칩을 누르면 그 일정을 고친다. 편집기는 날짜·시간·
  내용 세 칸이고 Enter로 저장한다. 시간을 비우면 '종일'로 표시된다. 내용을 비우고 저장하면
  지워진다. 일정은 `d`+`t` 순으로 정렬해 저장한다.
...
- **보기 모드**: 항목마다 `data-view="small|thumb"`. 썸네일은 파일 메모와 같은 셸 추출
  (`files.requestThumbs` → `files.thumb`)을 쓴다. 복사본은 절대 경로를 모르므로
  `memofile.absPaths`로 먼저 받아 온다(메모 폴더가 옮겨질 수 있어 절대 경로는 저장하지 않는다).

#### 드롭이 겹치지 않게 나누는 규칙

리치 본문에는 drop 처리기가 셋 붙어 있다. **순서와 각자의 몫을 지켜야 한 번 떨어뜨린 것이
두 번 들어가지 않는다**:

| 순서 | 처리기 | 가져가는 것 |
|---|---|---|
| 1 | 3D (sticker.js) | `.glb/.gltf/.obj/.stl` — `stopImmediatePropagation`으로 뒤를 끊는다 |
| 2 | `mediaTools.handleDrop` | 이미지 (여러 장이면 전부) |
| 3 | 파일 (sticker.js) | 이미지·3D를 **뺀 나머지**. 동영상은 첨부로 들여와 `<video>`로, 그 밖에는 파일 항목으로 |

2번은 `preventDefault`만 하고 전파를 막지 않으므로 **3번이 스스로 걸러야 한다** — 예전에는
그림과 파일 항목이 함께 생겼다(실측으로 잡았다).
...
- **전역 단축키** (`settings.hotkeys`, `App::RegisterHotkeys`): 숨김 앱 창(`hwnd_`,
  ...
  `Win`도 수정자로 쓸 수 있다(실측: `Win+F8`·`Win+Shift+K` 등록 성공, `Win+L`은 실패).
  설정 화면의 캡처는 `KeyboardEvent`의 `metaKey`를 `Win`으로 옮기고, 표기는 윈도우
  관례대로 Win·Ctrl·Alt·Shift 순으로 잇는다(`ParseHotkey`는 순서를 따지지 않는다).
  - **보이기/감추기 규칙** (`ToggleShowAllFront`): 전경이 우리 프로세스가 아니면 맨
    앞으로만, 우리 것이면 화면에 나와 있는 창을 모두 감추고, 다음 누름에 그것들만
    되돌린다. **`data.hidden`을 건드리지 않는 것이 핵심**이다 — 그 값은 "×로 닫아 둔
    메모"를 뜻하고 사용자는 닫아 둔 메모까지 꺼내지길 원하지 않는다. 그래서 이 단축키가
    감춘 창은 `hotkeyHidden_`(스티커 id, 그룹은 `"g:"+id`)에 따로 적어 두고 그것만
    복원한다. 부수 효과로 단축키로 감춘 채 앱을 끝내도 다음 실행에 원래대로 열린다.
    트레이의 명시적 전체 보이기/감추기(`SetAllVisible`)는 이 기록을 지운다.
    **함정**: `SetForegroundWindow`는 다른 앱이 방금 입력을 받았으면 조용히 거부된다
