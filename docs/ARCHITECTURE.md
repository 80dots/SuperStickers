...
- **메모창 다중 선택** (세션 한정, 저장 안 함 — `App::selected_`): Shift+클릭으로 창을
  ...
  **주의**: 창 클래스에 CS_HREDRAW/CS_VREDRAW가 없어 크기가 바뀌면 새로 드러난 영역만
  다시 그려진다 → 테두리가 점선처럼 끊긴다(자동 숨김으로 창이 늘 때 실제 발생).
  그래서 선택 중에는 WM_SIZE에서 클라이언트 전체를 무효화해 테두리를 이어 준다.
  **또 하나의 함정**: 자동 숨김의 자식 뷰 고정(핀) 때문에 접히면 WebView 자식이 창의
  위·아래 밴드를 덮어 그 위에 그린 테두리가 가려진다(좌우만 남아 끊겨 보임). 그래서
  `ClipChildrenToBand()`가 접힘 중에는 자식 창을 `SetWindowRgn`으로 밴드 안쪽까지만
  잘라 밴드가 항상 드러나게 한다. 창 밖으로 나간 부분은 어차피 보이지 않으므로 화면
  결과는 같다. 펼친 상태에서는 영역을 해제(nullptr)해 원래대로 둔다.
